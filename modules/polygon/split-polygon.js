export default splitPolygon = (polygon, rayOrig, rayDir) => {
	...
	for(var ivert=0; ivert<polygon.length; ivert++) {
		...
		if(den!=0) {
			...
			if(s>=0 && s<=1) {
				...
				var t = num / den;
				interPoints.push({
					is:(ivert+polygon.length-1)%polygon.length,
					ie:ivert,
					p:p,
					t:t});
			}
...
//	console.log(interPoints);
	...
	for(let ivert=0; ivert<polygon.length; ivert++) {
//		console.log(ivert);
		//append first point to poly
		curPoly.push(polygon[ivert]);
